# The setup form: every question Monarch asks a human to describe their machine
...
# (monarch-provision-owner). Sourced by both, so the copies cannot drift the way
...
MONARCH_KEYBOARD_LAYOUTS=$'English (US)|us
English (UK)|uk
...
MONARCH_USERNAME_PATTERN='^[a-z_][a-z0-9_-]*[$]?$'
MONARCH_RESERVED_USERNAMES='^(root|bin|daemon|mail|ftp|http|nobody|dbus|systemd-coredump|systemd-network|systemd-oom|systemd-journal-remote|systemd-resolve|systemd-timesync|tss|uuidd|alpm|git|avahi|cups|lp|_talkd|polkitd|rtkit|qemu|brltty|gluster|rpc|libvirt-qemu|pcscd|nvidia-persistenced|sddm)$'
MONARCH_HOSTNAME_PATTERN='^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?$'
MONARCH_HOSTNAME_DEFAULT='monarch'
...
monarch_username_taken() { return 1; }
...
monarch_prompt_keyboard() {
  local choice status
  choice=$(printf '%s\n' "$MONARCH_KEYBOARD_LAYOUTS" | cut -d'|' -f1 |
    gum choose --height 10 --selected "French" --header "Select keyboard layout") && status=0 || status=$?
  ((status == 0)) || return $status

  keyboard_label="$choice"
  keyboard=$(printf '%s\n' "$MONARCH_KEYBOARD_LAYOUTS" | awk -F'|' -v c="$choice" '$1==c{print $2; exit}')
}

monarch_prompt_username() {
  local status
  while true; do
    username=$(gum input --placeholder "Alphanumeric without spaces (like alex)" --prompt.foreground="#845DF9" --prompt "Username> ") && status=0 || status=$?
    ((status == 0)) || return $status

    if [[ "$username" =~ $MONARCH_USERNAME_PATTERN ]]; then
      if [[ "$username" =~ $MONARCH_RESERVED_USERNAMES ]]; then
        notice "Username is reserved for system" 1
      elif monarch_username_taken "$username"; then
...
monarch_prompt_identity() {
  local status
  full_name=$(gum input --placeholder "Used for Git author identity (hit return to skip)" --prompt.foreground="#845DF9" --prompt "Full name> ") && status=0 || status=$?
  ((status == 0)) || return $status
  email_address=$(gum input --placeholder "Used for Git author identity (hit return to skip)" --prompt.foreground="#845DF9" --prompt "Email address> ") && status=0 || status=$?
  return $status
}

monarch_prompt_hostname() {
  local status
  while true; do
    hostname=$(gum input --placeholder "Letters, digits, and dashes (or return for 'monarch')" --prompt.foreground="#845DF9" --prompt "Hostname> ") && status=0 || status=$?
    ((status == 0)) || return $status

    if [[ -z $hostname ]]; then
      hostname="$MONARCH_HOSTNAME_DEFAULT"
      return 0
    elif [[ "$hostname" =~ $MONARCH_HOSTNAME_PATTERN ]]; then
      return 0
    else
