run_logged "$MONARCH_INSTALL/config/theme-system.sh"
run_logged "$MONARCH_INSTALL/config/browser-policy.sh"
run_logged "$MONARCH_INSTALL/config/increase-lockout-limit.sh"
run_logged "$MONARCH_INSTALL/config/fix-powerprofilesctl-shebang.sh"
run_logged "$MONARCH_INSTALL/config/ssh-command-path.sh"
run_logged "$MONARCH_INSTALL/config/ssh-keepalive.sh"
run_logged "$MONARCH_INSTALL/config/docker.sh"
run_logged "$MONARCH_INSTALL/config/snapper.sh"
run_logged "$MONARCH_INSTALL/reconcile/cups-browsed.sh"
run_logged "$MONARCH_INSTALL/reconcile/fuzzel.sh"
run_logged "$MONARCH_INSTALL/config/enable-services.sh"
run_logged "$MONARCH_INSTALL/config/firewall.sh"
