# Remap Copilot key (Super+Shift+F23) to Super+Alt+Space (Monarch Menu) using makima
mkdir -p "$HOME/.config/makima"
cp "$MONARCH_PATH/default/makima/AT Translated Set 2 keyboard.toml" "$HOME/.config/makima/"
