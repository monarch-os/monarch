...
EOF
  ...
  sudo cp $MONARCH_PATH/default/limine/default.conf /etc/default/limine
  ...
  sudo cp $MONARCH_PATH/default/limine/limine.conf /boot/limine.conf

  # Only snapshot root — /home is user data; rolling it back loses user work
  if ! sudo snapper list-configs 2>/dev/null | grep -q "root"; then
    sudo snapper -c root create-config /
  fi
  sudo cp $MONARCH_PATH/default/snapper/root /etc/snapper/configs/root
