--
-- Dynamic Monarch Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the monarch-shipped Plymouth via
-- monarch-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs monarch-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "monarchunlocks"
NamePretty = "Monarch Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/monarch/themes"
  local monarch_path = os.getenv("MONARCH_PATH") or ""
  local default_themes_dir = monarch_path .. "/themes"
  local default_preview = monarch_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "monarch-launch-floating-terminal-with-presentation "
                .. shell_escape("monarch-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "monarch-launch-floating-terminal-with-presentation "
        .. shell_escape("monarch-plymouth-reset"),
    },
  }
