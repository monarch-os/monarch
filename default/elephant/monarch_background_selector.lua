Name = "monarchBackgroundSelector"
NamePretty = "Monarch Background Selector"
Cache = false
...
function GetEntries()
  ...
  local theme_name_file = io.open(home .. "/.config/monarch/current/theme.name", "r")
  ...
  local dirs = {
    home .. "/.config/monarch/current/theme/backgrounds",
  }
  if theme_name then
    table.insert(dirs, home .. "/.config/monarch/backgrounds/" .. theme_name)
  end
      ...
      for background in handle:lines() do
        ...
        if filename and not seen[filename] then
          ...
          table.insert(entries, {
            ...
            Value = background,
            Actions = {
              activate = "monarch-theme-bg-set " .. ShellEscape(background),
            },
            Preview = background,
            PreviewType = "file",
          })
