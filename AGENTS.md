...
- No user templates: `[templates.<name>]` with `input_path`/`output_path`/`post_hook` is valid only inside a template's own `template.toml`, under the builtin (`/usr/share/noctalia/assets/templates`) or community (`~/.local/state/noctalia/community-templates`) tree. In the user config it is rejected as an unknown section. Apps with no catalog entry are Monarch's problem, but not always Monarch's work: herdr solves it for free by querying the terminal for its palette over OSC, so its config names colours by ANSI slot and is never re-rendered. SDDM is outside the catalog; `monarch-sddm-apply` renders its `theme.conf` and logo from `monarch-theme-colors`.
- Residual layer: the only remaining Monarch piece is `bin/monarch-theme-apply` (`monarch theme apply`), wired to Noctalia's `[hooks] colors_changed`. It re-applies what Noctalia can't reach — wallpaper folder, keyboard RGB (asusctl/qmk_hid), Chromium policy color, and SDDM — reading the active scheme via `noctalia msg color-scheme-get` / `theme-mode-get`. Plymouth remains interactive because applying it rebuilds the initramfs. It never writes the theme mode.
