...
if [[ ${MONARCH_WINDOWS_TEST_NAMESPACE:-0} != 1 ]]; then
  if unshare --user --map-current-user --keep-caps --mount true 2>/dev/null; then
    exec env MONARCH_WINDOWS_TEST_NAMESPACE=1 \
...
TMPDIR=$(mktemp -d)
export MONARCH_WINDOWS_DIR="$TMPDIR/win"
export HOME="$TMPDIR/home"
mkdir -p "$HOME"

set -- help
source "$ROOT/bin/monarch-windows-vm" >/dev/null 2>&1
COMPOSE="$MONARCH_WINDOWS_DIR/docker-compose.yml"
TEST_HOME=$HOME
...
write() { # RAM CORES DISK USER PASS TZ [PRODUCT_KEY]
  printf 'RAM=%s\nCORES=%s\nDISK=%s\nUSERNAME=%s\nPASSWORD=%s\nTZ=%s\nPRODUCT_KEY=%s\n' \
    "$1" "$2" "$3" "$4" "$5" "$6" "${7:-}" | __priv_write_compose
}
...
reset_case() {
  unmount_all
  rm -rf "$MONARCH_WINDOWS_DIR" "$HOME/.windows" "$HOME/Windows"
  mkdir -p "$HOME"
}
...
grep -q 'image: dockurr/windows' "$COMPOSE" || fail "image is not fixed"
grep -q 'container_name: monarch-windows' "$COMPOSE" || fail "container name is not fixed"
grep -q -- '- NET_ADMIN' "$COMPOSE" || fail "cap_add is not fixed"
grep -q -- '- 127.0.0.1:8006:8006' "$COMPOSE" || fail "web console is not loopback-only"
grep -q 'restart: "no"' "$COMPOSE" || fail "VM can restart without verified mounts"
...
if command -v docker-compose >/dev/null 2>&1; then
  docker-compose -f "$COMPOSE" config >/dev/null || fail "generated compose does not parse"
fi
grep -Eq '^x-monarch-storage-fingerprint: "[0-9a-f]{64}"$' "$COMPOSE" ||
  fail "compose lacks a persistent storage fingerprint"
grep -Eq '^x-monarch-shared-fingerprint: "[0-9a-f]{64}"$' "$COMPOSE" ||
  fail "compose lacks a persistent shared fingerprint"
...
product_key=H7TFB-N4W6M-6QVGX-9CBTV-KHKQY
write 8G 4 64G bob "$tricky" UTC "$product_key"
...
grep -qF "KEY: \"$product_key\"" "$COMPOSE" || fail "valid OEM key was not retained"
rm -f "$COMPOSE"
printf 'RAM=4G\nCORES=2\nDISK=64G\nUSERNAME=ok\nPASSWORD=p\nTZ=UTC\nPRODUCT_KEY=bad:key\n' |
  __priv_write_compose 2>/dev/null && fail "invalid OEM key accepted"
...
mkdir -p "$attack_bin"
ln -s /bin/bash "$attack_bin/monarch-windows-vm"
printf 'printf exploited >"$TMPDIR/exploited"\n' >"$TMPDIR/__priv"
stat() {
  [[ ${!#} == /usr/share/monarch/bin/monarch-windows-vm ]] && return 1
  command stat "$@"
}
...
pass "pkexec target is only the packaged regular file, never a PATH symlink"

docker_socket="$TMPDIR/docker.sock"
MONARCH_DOCKER_SOCKET="$docker_socket" docker_needs_sudo ||
  fail "missing Docker socket bypassed polkit"
touch "$docker_socket"
chmod 0600 "$docker_socket"
if MONARCH_DOCKER_SOCKET="$docker_socket" docker_needs_sudo; then
  fail "direct Docker access still required polkit"
fi
pass "Docker socket access selects the privileged or direct path"

bad_option_compose="$TMPDIR/bad-option-compose"
if (COMPOSE_FILE="$bad_option_compose"; launch_windows --unexpected) >/dev/null 2>&1; then
  fail "unknown launch option was accepted"
fi
[[ ! -e $bad_option_compose ]] || fail "unknown launch option changed VM state"
pass "unknown launch options fail before VM state changes"
...
# Free-running swaps can fail preflight without ever reaching Docker.
# Hold the substituted path until every mount assertion has completed.
reset_case
prepare_user_mount_sources
...
mkfifo "$TMPDIR/race-command" "$TMPDIR/race-reply"
exec {race_command_fd}<>"$TMPDIR/race-command"
exec {race_reply_fd}<>"$TMPDIR/race-reply"
(
  read -r -t 10 -u "$race_command_fd" command && [[ $command == "swap" ]] || exit 1
  mv -T -- "$HOME/Windows" "$race_source"
  ln -s / "$HOME/Windows"
  printf 'swapped\n' >&"$race_reply_fd"
  read -r -t 10 -u "$race_command_fd" command && [[ $command == "restore" ]] || exit 1
  rm -- "$HOME/Windows"
  mv -T -- "$race_source" "$HOME/Windows"
  printf 'restored\n' >&"$race_reply_fd"
) &
racer_pid=$!
...
dc() {
  local reply
  [[ $1 == up && ${2:-} == -d ]] || return 1
  printf 'swap\n' >&"$race_command_fd" || return 1
  read -r -t 10 -u "$race_reply_fd" reply && [[ $reply == "swapped" ]] || return 1
  ...
  [[ -f $EXPECTED_SHARED/safe-marker ]] || return 1
  printf 'restore\n' >&"$race_command_fd" || return 1
  read -r -t 10 -u "$race_reply_fd" reply && [[ $reply == "restored" ]]
}
__priv_up || fail "concurrent path swap failed to complete bring-up with the pinned mounts"
wait "$racer_pid" || fail "concurrent attacker failed"
racer_pid=""
exec {race_command_fd}>&-
exec {race_reply_fd}>&-
unset -f dc docker
((concurrent_dc_calls == 1)) || fail "concurrent race did not reach Docker exactly once while the familiar path named host root"
[[ ! -L $HOME/Windows && $(stat -Lc '%d:%i' "$HOME/Windows") == "$concurrent_shared_id" ]] || fail "concurrent attacker did not restore the shared source"
...
reset_case
prepare_user_mount_sources
write 4G 2 64G recorded pw UTC
touch "$HOME/.windows/recorded.img" "$HOME/Windows/recorded.txt"
unmount_all
mv "$HOME/.windows" "$HOME/recorded-storage"
mv "$HOME/Windows" "$HOME/recorded-shared"
mkdir "$HOME/.windows" "$HOME/Windows"
touch "$HOME/.windows/unrelated" "$HOME/Windows/unrelated"
__priv_remove 2>/dev/null && fail "removal accepted storage that differs from its persistent identity"
[[ -f $HOME/.windows/unrelated && -f $HOME/Windows/unrelated && -f $COMPOSE ]] ||
  fail "identity rejection changed unrelated data or compose"
pass "removal refuses familiar paths retargeted after bind anchors disappear"

reset_case
prepare_user_mount_sources
write 4G 2 64G unrecorded pw UTC
sed -i '/^x-monarch-.*-fingerprint:/d' "$COMPOSE"
__priv_remove 2>/dev/null && fail "removal accepted a compose without persistent source identities"
[[ -f $COMPOSE ]] || fail "unrecorded removal changed compose state"
pass "removal refuses legacy protected state until a non-destructive migration records identity"
...
unsafe_config_target="$TMPDIR/unsafe-config-target"
unsafe_config_link="$TMPDIR/unsafe-config-link"
mkdir "$unsafe_config_target"
ln -s "$unsafe_config_target" "$unsafe_config_link"
CREDENTIALS_FILE="$unsafe_config_link/credentials"
write_credentials attacker replaced 2>/dev/null && fail "symlinked config directory accepted credentials"
[[ ! -e $unsafe_config_target/credentials ]] || fail "credential write followed config directory symlink"
CREDENTIALS_FILE="$credentials_dir/credentials"
pass "private configuration refuses a symlinked directory"
...
cleanup_root="$TMPDIR/cleanup"
cleanup_target="$TMPDIR/cleanup-storage-target"
mkdir -p "$cleanup_root/config" "$cleanup_target"
CREDENTIALS_FILE="$cleanup_root/config/credentials"
HOME="$cleanup_root/home"
mkdir -p "$HOME/.local/share/applications"
ln -s "$cleanup_target" "$HOME/.windows"
printf secret >"$cleanup_root/config/credentials"
printf keep >"$cleanup_root/config/unmanaged"
cleanup_user_vm_files
[[ ! -L $HOME/.windows && -d $cleanup_target ]] || fail "cleanup removed an external storage target"
[[ ! -e $cleanup_root/config/credentials ]] || fail "cleanup kept managed credentials"
[[ -f $cleanup_root/config/unmanaged ]] || fail "cleanup removed an unmanaged config file"
HOME=$TEST_HOME
CREDENTIALS_FILE=$DEFAULT_CREDENTIALS_FILE
pass "user cleanup removes only recorded leaves and the familiar storage link"

unconfigured="$TMPDIR/unconfigured"
mkdir -p "$unconfigured/.windows"
printf keep >"$unconfigured/.windows/disk.img"
if (
  HOME="$unconfigured"
  COMPOSE_FILE="$unconfigured/protected-compose"
  LEGACY_COMPOSE_FILE="$unconfigured/legacy-compose"
  CREDENTIALS_FILE="$unconfigured/.config/windows/credentials"
  remove_windows
) >/dev/null 2>&1; then
  fail "removal without a recorded compose succeeded"
fi
[[ -f $unconfigured/.windows/disk.img ]] || fail "unconfigured removal deleted storage"
! grep -qE 'rm -rf .*\.(windows|config/windows)' "$ROOT/bin/monarch-windows-vm" ||
  fail "Windows VM cleanup still uses a reconstructed recursive path"
pass "removal without trusted state leaves storage untouched"

grep -qF '/from-stdin:force' "$ROOT/bin/monarch-windows-vm" ||
  fail "RDP password remains on the process command line"
! grep -qE 'xfreerdp3 .*/p:' "$ROOT/bin/monarch-windows-vm" ||
  fail "RDP command still embeds its password argument"
pass "RDP reads its password from standard input"
