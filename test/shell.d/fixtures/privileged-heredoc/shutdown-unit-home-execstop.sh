...
touch "$HOME/.config/monarch/.plymouth-sync-needed"

# Create the systemd service
sudo tee /etc/systemd/system/monarch-plymouth-shutdown.service >/dev/null <<EOF
[Unit]
...
RemainAfterExit=yes
ExecStart=/bin/true
ExecStop=$HOME/.local/share/monarch/bin/monarch-plymouth-shutdown-sync
...
sudo systemctl enable monarch-plymouth-shutdown.service
sudo systemctl start monarch-plymouth-shutdown.service
