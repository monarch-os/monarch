...
cp "$ROOT/test/fixtures/noctalia-v4/herdr.toml" \
  "$HOME/.config/noctalia/templates/herdr.toml"
cat >>"$HOME/.config/noctalia/templates/herdr.toml" <<'EOF'
...
sed -e 's/{{colors.surface_container.default.hex}}/#112233/' \
  -e 's/{{colors.primary.default.hex}}/#ABCDEF/' \
  "$HOME/.config/noctalia/templates/herdr.toml" >"$HOME/.config/herdr/config.toml"
cp "$HOME/.config/herdr/config.toml" "$test_tmp/custom-v4-herdr-original"
cp "$HOME/.config/noctalia/templates/herdr.toml" "$test_tmp/custom-v4-herdr-template"
bash "$ROOT/install/reconcile/schema/1-to-2/legacy-noctalia.sh"
cmp "$test_tmp/custom-v4-herdr-original" "$HOME/.config/herdr/config.toml"
cmp "$test_tmp/custom-v4-herdr-template" \
  "$HOME/.config/noctalia/templates/herdr.toml"
...
pass "custom V4 Herdr settings and template survive"
