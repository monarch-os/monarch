...
output=$("$CLI" --help)
assert_output_contains "main help renders" "$output" "Monarch command center"
...
assert_output_contains "restart group includes inferred commands" "$output" "monarch restart btop"
assert_output_contains "restart group includes all restart commands" "$output" "monarch restart wifi"
...
assert_output_contains "bare share help uses canonical route" "$output" "monarch share <clipboard|file|folder> [path...]"
...
assert_output_contains "bare required-choice route renders CLI help" "$output" "monarch branch set <main|dev>"
...
for command in commands:
  ...
  stem = binary.removeprefix('monarch-')
...
assert_output_contains "root command help shows related child commands" "$output" "monarch update perform"
...
while IFS= read -r binary_path; do
  ...
  grep -q '^# monarch:summary=' <<<"$header" || fail "metadata summary is present: $binary_path"
  ! grep -q '^# monarch:binary=' <<<"$header" || fail "metadata does not repeat inferred binary: $binary_path"
  ! grep -q '^# monarch:args=$' <<<"$header" || fail "metadata does not include empty args: $binary_path"
  ! grep -Eq '^# monarch:(legacy|usage|visibility|mutates|interactive)=' <<<"$header" || fail "metadata avoids removed fields: $binary_path"
  ! grep -Eq '^# monarch:requires-sudo=false$' <<<"$header" || fail "metadata omits false booleans: $binary_path"
done < <(find "$ROOT/bin" -maxdepth 1 -type f -executable -name 'monarch-*' | sort)
pass "all executable bins have slim self-documenting metadata"

TMPDIR=$(mktemp -d)
ln -s "$CLI" "$TMPDIR/monarch"

{
  printf '#!/bin/bash\n\n'
  printf '# ordinary comments are fine\n'
  printf '# monarch:this malformed line should be ignored\n'
  printf '# monarch:group=weird\n'
  printf '# monarch:name=test\n'
  printf '# monarch:summary=Survives malformed metadata comments\n'
  printf '# monarch:made-up=value\n'
  printf 'echo weird-ok\n'
} >"$TMPDIR/monarch-weird-test"
chmod +x "$TMPDIR/monarch-weird-test"

{
  printf '#!/bin/bash\n\n'
  printf '# a partial metadata header should not destroy fallback routing\n'
  printf '# monarch:summary=Partial metadata keeps inferred route\n'
  printf '# monarch:made-up=value\n'
  printf 'echo partial-ok\n'
} >"$TMPDIR/monarch-partial-meta-test"
chmod +x "$TMPDIR/monarch-partial-meta-test"

{
  printf '#!/bin/bash\n\n'
  printf 'echo body-metadata-ok\n'
  printf '# monarch:group=wrong\n'
  printf '# monarch:name=wrong\n'
} >"$TMPDIR/monarch-body-metadata-test"
chmod +x "$TMPDIR/monarch-body-metadata-test"

"$TMPDIR/monarch" commands --all --json | jq -e '.commands[] | select(.route == "monarch weird test" and .summary == "Survives malformed metadata comments")' >/dev/null
pass "unknown metadata values are non-fatal"

"$TMPDIR/monarch" commands --all --json | jq -e '.commands[] | select(.route == "monarch partial meta test" and .summary == "Partial metadata keeps inferred route")' >/dev/null
pass "partial metadata keeps inferred fallback route"

"$TMPDIR/monarch" commands --all --json | jq -e '.commands[] | select(.route == "monarch body metadata test" and .summary == "Run the body metadata test command")' >/dev/null
pass "metadata-looking comments after script body are ignored"

output=$("$TMPDIR/monarch" weird test)
assert_output_contains "temporary metadata command dispatches" "$output" "weird-ok"

output=$("$TMPDIR/monarch" partial meta test)
assert_output_contains "partial metadata command dispatches" "$output" "partial-ok"

output=$("$TMPDIR/monarch" body metadata test)
