...
# Users can still opt-in to Cloudflare DNS using: monarch-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-monarch-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-monarch-dns.network
  ...
  echo "To use Cloudflare DNS, run: monarch-setup-dns Cloudflare"
fi
