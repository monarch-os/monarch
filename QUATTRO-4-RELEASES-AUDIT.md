...
| Groupe `input` global | `ff4cf8afb`, suppression de `install/hardware/input-group.sh` | **Couvert** : le grant par défaut et celui de l'installateur Xbox sont retirés; le provisionnement filtre l'ancien état et la transition v4→v5 révoque l'appartenance, sauf opt-in `ydotool`. | — |
